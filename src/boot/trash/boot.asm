...
;; do some lba to chs convertion
	mov word [fs_fat12.block], ax
	div word [BPB_SECPERTRACT]
	inc dl
	mov byte [fs_fat12.sector], dl

	mov ax, word [fs_fat12.block]
	mov cx, [BPB_SECPERTRACT]
	shl cx, 1
	div word cx
	mov byte [fs_fat12.track], al

	xchg dl, al
	div word [BPB_SECPERTRACT]
	mov byte [fs_fat12.head], al

	mov ah, 0x02
	mov al, 0x01
	mov ch, byte [fs_fat12.track]
	mov cl, byte [fs_fat12.sector]
	mov dh, byte [fs_fat12.head]
...
.next:
	...
	loop read	;; loop according to cx
	ret
